...
**Event Stream as primary truth.** Occurrences are recorded as immutable Events in a canonical Event Stream. State is a derived projection: a deterministic function of the Event Stream and configuration. The Event Stream is the infrastructure's truth; current state is recomputable from it.
...
2. **The Event Stream is the authoritative history.** Events are immutable once appended. The Event Stream is totally ordered by Processing Order — the strict internal causal sequence that determines when each Event is applied. Event Time (the external timestamp an Event carries) is metadata; it does not override Processing Order.
...
4. **State is reconstructible.** Given an identical Event Stream and identical Configuration, the infrastructure produces identical State at every Processing Order position. State can be reconstructed at any point by replaying the Event Stream under the same Configuration.
...
| **Control State** | Control Events (configuration changes, operational signals) |
...
**Deterministic replay is a structural property, not an aspiration.** Because State is fully determined by Event Stream + Configuration, and because no mutation path exists outside Event processing, replay of the same Event Stream under the same Configuration is guaranteed to produce the same State at every position. This holds for all State domains including Execution Control substate.
