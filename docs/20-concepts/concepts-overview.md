...
The infrastructure is **deterministic**: given an identical Event Stream, identical Configuration, and the same Processing Order, it produces identical State at every Event Stream position.
...
- **Venue** responses re-enter the Event Stream as **Execution Events**, advancing the Order through its lifecycle and returning to State derivation.
