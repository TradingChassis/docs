...
2. **Event Time does not determine Processing Order.** Two Events with the same Event Time are still ordered by their **Event Stream position** ([Event Model](event-model.md)).
