...
This document describes how the Monitoring Stack behaves during normal operation: how observability signals are collected from running infrastructure parts, how health and status become visible, how alert-oriented monitoring arises from ongoing conditions, and how operational visibility is maintained while the Infrastructure is running.
...
The strongest signal relationships exist with the **Live Stack** and the **Backtesting Stack**, where runtime observability is most operationally critical. Other running infrastructure parts may also emit signals toward the Monitoring Stack where observability support is needed.

Signal collection is passive with respect to the observed infrastructures — the Monitoring Stack receives and integrates signals without altering the execution logic, processing decisions, or runtime behavior of the Stacks it observes. Running infrastructure parts retain full ownership of their execution semantics. The Monitoring Stack provides visibility into that execution, not influence over it.

---

## Health, Status, and Telemetry During Operation

During active infrastructure execution, the Monitoring Stack maintains structured representations of health, status, and telemetry for monitored components.
...
Health, status, and telemetry are maintained as a **current operational picture** — a structured, continuously updated representation of how monitored infrastructure parts are behaving right now. This is the operational core of the Monitoring Stack's runtime behavior.
...
The Monitoring Stack exposes operational visibility surfaces that make running infrastructure behavior accessible during active execution.
...
- **Routine awareness.** During normal operation, visibility surfaces confirm that infrastructures are running nominally — metrics are within expected ranges, health is positive, no alert conditions are active. This is the baseline operational behavior: the Monitoring Stack provides assurance that running infrastructure parts are behaving as expected.
...
Operational visibility is **runtime-continuous**. It does not require a triggering event to become active — it is the normal operating state of the Monitoring Stack. Visibility surfaces are available throughout infrastructure execution, whether conditions are nominal or anomalous.
