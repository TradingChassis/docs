...
The Monitoring Stack decomposes into a set of logical capabilities that together accomplish one task: receiving observability-relevant signals from running system parts, structuring and evaluating them, and exposing the results as operational visibility, health views, and alert-oriented outputs.

The internal flow moves through four stages:

```mermaid
flowchart TB
    RS["Running System Parts"] --> TIL["Telemetry Ingestion Layer"]
...
All capabilities operate on signals from running systems — the Monitoring Stack does not consume persisted analytical outputs or historical experiment datasets. Its internal structure is oriented around ongoing, runtime-concurrent observation of active system behavior.
...
Receives observability-relevant signals from running system parts and normalizes them for downstream processing.
...
- Accept signals from multiple sources — primarily the Live Stack and the Backtesting Stack, and from other running system parts where observability support is needed.
...
The Telemetry Ingestion Layer is the entry point of the stack. It bridges running system parts and the Monitoring Stack's internal processing. No monitoring capability operates without ingested signals.
...
- Organize ingested signals by source, component, subsystem, or other structuring dimensions so that downstream capabilities can query and evaluate specific slices of operational data.
...
Evaluates aggregated signals to produce health assessments for monitored components and subsystems.
...
- Produce aggregated health views — per-component, per-subsystem, or system-wide — that represent the operational health posture of running system parts.
...
- Route alert-oriented outputs toward operators, external notification systems, or alert-handling infrastructure.
...
- Expose real-time and near-real-time views of running system behavior — metric time series, status summaries, health dashboards, and incident-relevant diagnostic views.
- Make the operational state of running system parts inspectable without requiring direct access to those systems.
...
1. **Signal ingestion.** The Telemetry Ingestion Layer receives observability-relevant signals from running system parts and normalizes them for internal processing.
...
3. **Health and alert evaluation.** The Health Evaluation Surface assesses component and subsystem health. The Alert Evaluation Surface evaluates aggregated data against monitoring conditions and produces alert-oriented outputs where noteworthy conditions are detected.
...
**No runtime execution.** The Monitoring Stack's internal structure observes running system behavior. It does not run Strategies, process Events, evaluate Risk, manage Execution Control, or interact with Venues. It makes execution visible — it does not perform it.
