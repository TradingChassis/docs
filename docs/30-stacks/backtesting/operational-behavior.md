...
2. The canonical dataset is read from Canonical Storage and transformed into the historical Event Stream that the Core Runtime will process. This supplies the historical market data (order book updates, trades, associated metadata) in the Processing Order required for deterministic replay.
